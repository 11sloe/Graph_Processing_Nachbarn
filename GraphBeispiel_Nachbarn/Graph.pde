...
{ 
  ...
  void nachbarnAusgeben(String ort, int farbe)
  {
    println("Nachbarn von " + ort + ":");
    int nummer = knotennummerGeben(ort);
    for (int i = 0; i < anzahl; i++)
    {
      if (matrix[nummer][i])
      {
        println(knotenliste[i].getInhalt());
        knotenliste[i].setFarbe(farbe);
      }
    }
  }
  
  void nachbarnVonNachbarn(String ort)
  {
    int nummer = knotennummerGeben(ort);
    for (int i = 0; i < anzahl; i++)
    {
      if(matrix[nummer][i])
      {
        nachbarnAusgeben(knotenliste[i].getInhalt(), color(255,255,0));
      }
    }
  }
  ...
  public void kanteEinfuegen(String a, String b)
  {
    ...
    {
      matrix[knoten1][knoten2] = true;
      matrix[knoten2][knoten1] = true;
    }
    else
    {
  ...
  private void knotenZeichnen()
  {
    ...
    for (int i = 0; i < anzahl; i++)
    {
      ...
      color farbe = knotenliste[i].getFarbe();
  ...
  public void mousePressed()
  {
    ...
    for (int i = 0; i < anzahl; i++)
    {
      ...
      if (dist(mouseX,mouseY,knotenliste[i].getX(),knotenliste[i].getY())<30)
      {
        ...
        nachbarnAusgeben(knotenliste[i].getInhalt(), color(0,255,0));
        //nachbarnVonNachbarn(knotenliste[i].getInhalt());
      }
    }
  }
}
